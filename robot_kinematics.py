...
class ANN:
    ...
    def fit_trainig_data(self, samples, features):
        ...
        #input_test_eval = scaler.transform(input_test_eval)
        ...
        print()
        print(np.array(input_test_eval).shape)
        print()
        print(np.array(output).shape)
        print()
        print(np.array(output_test_eval).shape)
...
if __name__ == '__main__':
    ...
    try:
        ...
        data_cube = generator.cube(step_size = 5, limits = limits)
    except Exception as e:
        print(str(e))

    # plot_points_3d([Point([*elem]) for elem in positions_samples])
    ...
    ann.model = load_model('roboarm_model_1664488076-610064.h5')

    # ANN
    print("ANN")
    predicted_points = []
    ik_angles_ann = ann.predict_ik(positions_samples).tolist()
    plot_points_3d([Point([*elem]) for elem in positions_samples])
    print('sample: ' + str(np.array(positions_samples)))
